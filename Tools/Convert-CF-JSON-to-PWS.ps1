...
# in eine TXT-Datei mit PowerShell-Feldzuweisungen im Format:
#     "Feldname"         = if ($Fields.id_CL) { $Fields.id_CL.ToString() } else { "leer" }
...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$OutputTxt = "C:\GitHub\Reinhold-Gr\JIRA-Cloud\Tools\CF_PWS.txt"
)
...
$equalsColumn = $maxLen + 1

$lines = foreach ($entry in $valid) {
    $quotedName = '"' + $entry.Feldname + '"'
    $padding    = ' ' * ($equalsColumn - $quotedName.Length)
    $fieldId    = $entry.id_CL
    "    $quotedName${padding}= if (`$Fields.$fieldId) { `$Fields.$fieldId.ToString() } else { `"leer`" }"
}
