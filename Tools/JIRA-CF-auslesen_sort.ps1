...
#    [string]$IssueKey     = "MITGLIEDER-243",
    [string]$IssueKey     = "RECHNUNG-8208",
...
$FieldMap = $Mapping |
    Where-Object { $null -ne $_.id_CL -and $_.id_CL -ne "" } |
    ForEach-Object {
        # Numerischen Teil aus id_CL extrahieren (z.B. "customfield_10380" -> 10380)
        $numPart = ($_.id_CL -replace '\D', '')
        [PSCustomObject]@{
            id_CL     = $_.id_CL
            Feldname  = $_.Feldname
            Nummer    = [int]$numPart
...
foreach ($entry in $FieldMap) {
    ...
    $keyLabel = "$($entry.Nummer) - $($entry.Feldname)"
    $Result[$keyLabel] = if ($value) { $value.ToString() } else { "leer" }
}
